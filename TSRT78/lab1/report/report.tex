...
Here, the observed data $x(t)$ is split into two
parts, the \textit{validation data} containing samples $1, 2, ..., N_1$, and
the \textit{estimation data} containing samples $N_1 + 1, N_1 + 2, ... ,N$,
...
the theory explained in Section~\ref{sec:whistletheory}. The \textit{minimal
loss function} $W(n)$ is calculated for each order $n$. By plotting $W(n)$, a
